  ...
  return (
    <div className="mx-auto flex min-h-[calc(100vh-8rem)] w-full max-w-[920px] flex-col gap-3 p-3">
      ...
      <div
        data-state={feel}
        className={cn(
          "relative flex min-h-[260px] flex-col items-center justify-center overflow-hidden border border-[var(--color-line)] bg-[linear-gradient(180deg,rgba(0,240,255,0.04),transparent_45%),var(--color-panel)] px-4 py-7 text-center",
        ...
        {feel !== "hit" ? (
          <>
            <div
              aria-hidden
              className={cn(
                "mb-4 h-16 w-16 rounded-full border-2 border-[rgba(0,240,255,0.35)] bg-[rgba(0,240,255,0.06)]",
      ...
      <section className="border border-[var(--color-line)] bg-[var(--color-panel)]">
        ...
        <div className="max-h-48 divide-y divide-[rgba(30,30,46,0.9)] overflow-auto">
          {events.length === 0 ? (
            <p className="px-3 py-3 text-sm text-[var(--color-muted-foreground)]">{t(locale, "common.waiting")}</p>
          ) : (
            events.slice(0, 12).map((ev) => (
              <div key={ev.id} className="flex items-start justify-between gap-3 px-3 py-2 text-sm">
