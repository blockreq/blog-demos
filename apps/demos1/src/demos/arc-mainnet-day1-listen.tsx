...
export function ArcMainnetDay1Demo({ locale }: { locale: Locale }) {
  ...
  const watchParams = [
    { label: t(locale, "arc.factoryLabel"), value: factory.trim() || (L(locale, "(unset)", "（待填）")), mono: true },
      ...
      <SettingsPanel open={showSettings}>
        <Card>
          <CardHeader className="pb-2">
            ...
            <CardDescription>
              {L(locale, "Pre-wire LAUNCHPAD_FACTORY + PairCreated topic0. Optional narrow Swap density on hit pair.", "预埋 LAUNCHPAD_FACTORY + PairCreated topic0。命中 pair 后可窄听 Swap 密度。")}
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                <Label htmlFor="arc-min">{L(locale, "Min early swaps", "最少换手数")}</Label>
                <Input id="arc-min" value={minSwaps} onChange={(e) => setMinSwaps(e.target.value)} onBlur={saveFields} />
              </div>
              <div className="space-y-1.5">
                <Label htmlFor="arc-win">{L(locale, "Window (sec)", "窗口（秒）")}</Label>
