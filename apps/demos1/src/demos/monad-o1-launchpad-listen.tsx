...
import { buildMonadO1Fixtures, useDemoHits } from "../lib/demo-hits";
...
/** Uniswap V2 PairCreated — editable for Monad o1 launchpad factory day-0. */
const TOPIC_PAIR =
...
const LS = "blockreq.monad-o1.";
const SLUG = "monad-o1-launchpad-listen";
...
/**
 * Monad o1 day-0 Factory PairCreated prep — endpoints empty until BlockReq Monad public ready.
 * Layout: launch-feed (continuous creates + optional swap density, same family as Arc day-1).
 */
export function MonadO1LaunchpadDemo({ locale }: { locale: Locale }) {
  ...
  const ep = useEditableEndpoints(SLUG, "monad");
  const epRef = useRef(ep);
  epRef.current = ep;

  const seedEvents = useMemo(() => buildMonadO1Fixtures(locale, 6), [locale]);
  const history = useRecentHistory({
    locale,
    https: ep.https,
    address: factory.trim() || undefined,
    topics: [topicPair],
    map: (logs) => mapPairCreatedLogs(logs, locale, "MONAD"),
  ...
  const injectDemoHits = useCallback(() => {
    const fixtures = buildMonadO1Fixtures(locale, 3);
  ...
  useEffect(() => {
    // Auto-live only when Monad WSS is filled; otherwise stay idle with placeholder hint.
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            <div className="space-y-1.5">
              <Label htmlFor="monad-factory">{t(locale, "monad.factoryLabel")}</Label>
              <Input
                id="monad-factory"
                placeholder="0x…"
                value={factory}
            ...
            <div className="space-y-1.5">
              <Label htmlFor="monad-tp">PAIR_CREATED_TOPIC0</Label>
              <Input id="monad-tp" value={topicPair} onChange={(e) => setTopicPair(e.target.value)} onBlur={saveFields} />
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="monad-ts">Swap topic0</Label>
              <Input id="monad-ts" value={topicSwap} onChange={(e) => setTopicSwap(e.target.value)} onBlur={saveFields} />
            </div>
            ...
            <p className="break-all font-mono text-[11px] text-[var(--color-muted-foreground)]">
              {ep.wss || "MONAD_WSS=''"} · {ep.chainIdHex}
            </p>
          </CardContent>
        </Card>
      </SettingsPanel>
  ...
  return (
    <div className="flex min-h-screen flex-col pb-24" data-layout="launch-feed">
      <MonitorChrome
        ...
        locale={locale}
        title={t(locale, "monad.title")}
        tag={t(locale, "monad.tag")}
        status={status}
        hasHit={hasHit}
        slug={SLUG}
      ...
      <AnonStreamLayout
        ...
        guide={t(locale, "monad.guide")}
        watching={t(locale, "monad.watching")}
        hint={t(locale, "monad.hint")}
        emptyTitle={t(locale, "monad.emptyTitle")}
        emptySub={t(locale, "monad.emptySub")}
        latestLabel={t(locale, "monad.latest")}
        chainBadge="Monad"
        endpointSlot={
          <EndpointBar
            locale={locale}
