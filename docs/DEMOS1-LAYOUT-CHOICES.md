...
Reuses MonitorChrome (centered header, BlockReq RPC - Blockchain Infrastructure CTA), collapsed RPC bar prefilled with **live** BlockReq BSC public HTTPS/WSS, and addr hover via shared chrome/feed components. Verified Flap Portal `0xe2cE6a…` + TokenCreated topic0 only — Pancake V2 factory is a hint chip, not the listen target. Zero Solana / placeholder / publicnode endpoints.
...
Reuses MonitorChrome (centered header, BlockReq RPC - Blockchain Infrastructure CTA), collapsed RPC bar prefilled with **live** BlockReq Base public HTTPS/WSS, and addr hover via shared chrome/feed components. Verified StonkLauncher2 factory `0x4714f6…` + TokenLaunched topic0 only — never BaseStonk `0x74655…`. Zero Solana / placeholder / publicnode endpoints.
...
Reuses MonitorChrome (centered header, BlockReq RPC - Blockchain Infrastructure CTA), collapsed RPC bar prefilled with **live** BlockReq Base public HTTPS/WSS, and addr hover via shared chrome/feed components. Verified vault / $RWA / USDC / topic0s only — Aerodrome RWA/USDC is a DEX chart chip, not a listen target. Zero Solana / Arc / Robinhood / publicnode / placeholder endpoints.
...
Both demos reuse MonitorChrome (centered header, BlockReq RPC - Blockchain Infrastructure CTA), editable RPC bar prefilled with **live** BlockReq BSC / Arbitrum One public HTTPS/WSS, and addr hover via shared chrome/feed components. Factory / RWA / stable lists use labeled editable samples — RPC defaults are live public only (zero Solana / fake endpoints).
...
Both demos reuse MonitorChrome (centered header, BlockReq RPC - Blockchain Infrastructure CTA), editable RPC bar, and addr hover via shared chrome/feed components. Empty `PUBLIC_ENDPOINTS.solana` placeholders from #26 — never hardcode a live `*.blockreq.com` Solana public URL.
...
Both demos reuse MonitorChrome (centered header, BlockReq RPC - Blockchain Infrastructure CTA), editable RPC bar, and addr hover via shared chrome/feed components.
